...
test("คำถามภูมิแพ้และโรคกระเพาะใน OPD แนบลิงก์ระยะรอคอย", () => {
  ...
  assert.match(reply, /180 วัน/);
  assert.doesNotMatch(reply, /120 วัน/);
});

test("คำถามความรู้อย่างเดียวไม่เข้าสู่ขั้นถามข้อมูลทำใบเสนอราคา", () => {
  assert.equal(
    isKnowledgeOnlyTurn(
      {
        intent: "insurance_advice",
        asksForPremium: false,
        shouldRecommendPlan: false,
        knowledgeTopics: [],
      },
      "โรคภูมิแพ้กับโรคกระเพาะในแผน OPD มีระยะรอคอยกี่วันครับ"
    ),
    true
  );
    ...
    isKnowledgeOnlyTurn(
      {
        intent: "insurance_advice",
        asksForPremium: true,
        shouldRecommendPlan: true,
        knowledgeTopics: ["waiting_period"],
      },
      "สนใจแผน OPD ช่วยคำนวณเบี้ยและบอกระยะรอคอยให้ด้วยครับ"
    ),
    false
  );
});
