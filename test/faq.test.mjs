...
test("สาระสำคัญที่หาข้อมูลมาต้องอยู่ครบ ไม่ถูกตัดทิ้งตอนย่อ", () => {
  /* เดิมเขียนว่าแต่ละแบบสืบย้อนได้ไม่เท่ากัน ซึ่งไม่ถูก
     การขอประวัติไม่มีเพดานปี ที่ต่างกันคือผลที่บริษัทเอาไปใช้ได้ จึงต้องเขียนแยกเป็นชั้น */
  assert.match(html, /การขอประวัติการรักษาไม่มีเพดานปีสำหรับสัญญาไหนเลย/);
  assert.match(html, /ป\.พ\.พ\. มาตรา 865/);
  assert.match(html, /ภายใน 1 เดือนนับจากวันที่บริษัททราบมูล/);
  assert.match(html, /ไม่เกิน 5 ปีนับจากวันทำสัญญา/);
  // ตัวเลข 2 ปีมาจากข้อไม่โต้แย้งในกรมธรรม์ ไม่ได้มาจากกฎหมาย ต้องบอกให้ชัด
  assert.match(html, /ข้อไม่โต้แย้งความสมบูรณ์ของสัญญาไว้สั้นกว่ากฎหมาย เหลือ 2 ปี/);
  // จุดที่คนเข้าใจผิดมากที่สุด คือคิดว่าพ้น 2 ปีแล้วปลอดภัยทุกเรื่อง
  assert.match(html, /ชั้นนี้ไม่หมดอายุพร้อมกับชั้นที่ 1/);
  assert.match(html, /5 ปีก่อน และ 3 ปีหลัง/);
  // ระยะรอคอยสองชั้น
...
/* เกณฑ์ Simple Diseases คือสิ่งที่ตัดสินว่าเคสนั้นเข้าเกณฑ์ Copayment แบบ 200% หรือ 400%
   ตัวเลขสัญญาณชีพทุกตัวจึงมีผลต่อเงินจริง ห้ามพิมพ์ผิดแม้ตัวเดียว
   อ้างอิงประกาศมาตรฐานประกันสุขภาพแบบใหม่ของ คปภ. ลงวันที่ 8 พฤศจิกายน 2564 */
test('ต้องมีหัวข้อ Simple Diseases พร้อมรายชื่อโรคและเกณฑ์สัญญาณชีพครบ', () => {
  assert.match(html, /k:'simple-diseases'/);
  const start = html.indexOf("k:'simple-diseases'");
  const item = html.slice(start, start + 4200);

  for(const d of ['ติดเชื้อทางเดินหายใจส่วนบน','ไข้หวัดใหญ่','ท้องเสีย','เวียนศีรษะ',
                  'ไข้ไม่ระบุสาเหตุ','ปวดหัว','กล้ามเนื้ออักเสบ','ภูมิแพ้',
                  'โรคกระเพาะอาหารอักเสบและกรดไหลย้อน'])
    assert.ok(item.includes(d), `ขาดโรค ${d}`);

  for(const c of ['36 ถึง 38.5 องศาเซลเซียส','50 ถึง 120 ครั้งต่อนาที','9 ถึง 22 ครั้งต่อนาที',
                  '90 ถึง 180 มิลลิเมตรปรอท','60 ถึง 110 มิลลิเมตรปรอท','เกินกว่า 95%',
                  'คะแนนความปวดน้อยกว่า 7','ขาดน้ำเพียงเล็กน้อย'])
    assert.ok(item.includes(c), `ขาดเกณฑ์ ${c}`);

  // สองชั้นต้องเข้าครบทั้งคู่ ถ้าเขียนแค่ชื่อโรคลูกค้าจะเข้าใจผิดว่าเป็นโรคนี้แล้วโดนแน่
  assert.ok(item.includes('ต้องเข้าครบทั้ง 2 ชั้น'), 'ต้องบอกว่าดูแค่ชื่อโรคไม่พอ');
  assert.ok(item.includes('8 พฤศจิกายน 2564'), 'ต้องอ้างอิงวันที่ประกาศ');
  assert.ok(item.includes('เด็กอายุ 3 ถึง 5 ปี'), 'ต้องบอกว่ารายการของเด็กสั้นกว่า');
});

test('ข้อ Copayment ต้องโยงไปหากลุ่ม Simple Diseases ไม่ใช่พูดลอย ๆ ว่ากลุ่มโรคเล็กน้อย', () => {
  assert.match(html, /กลุ่มการป่วยเล็กน้อยทั่วไป \(Simple Diseases\)/);
  const i = html.indexOf('กลุ่มการป่วยเล็กน้อยทั่วไป (Simple Diseases)');
  assert.ok(html.slice(i, i + 700).includes('200%'), 'ต้องบอกเกณฑ์ 200% ที่ต่างจากโรคทั่วไป');
});
