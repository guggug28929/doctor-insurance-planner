...
test('ทุกข้อต้องบอกทั้งฝั่งที่เข้าข้อยกเว้นและฝั่งที่ไม่ควรเข้า', () => {
  ...
  for(const k of both){
    const b = faqBody(k);
    // ทุกข้อรายโรคต้องมีการ์ดครบทั้งสามสี ไม่งั้นลูกค้าจะสรุปเอียงไปด้านใดด้านหนึ่ง
    assert.ok(/^@R /m.test(b), `${k} ไม่มีการ์ดสีแดงบอกว่าอะไรเบิกไม่ได้`);
    assert.ok(/^@G /m.test(b), `${k} ไม่มีการ์ดสีเขียวบอกว่าอะไรยังเบิกได้`);
    // ทุกการ์ดต้องบอกทางแก้ ไม่ใช่บอกแค่ว่าเบิกไม่ได้แล้วปล่อยลูกค้าไว้กลางทาง
    for(const line of b.split('\n').filter(l => /^@[RYG] /.test(l)))
      assert.equal(line.split('::').length, 3, `${k} มีการ์ดที่ไม่ได้เขียนครบสามช่อง: ${line.slice(0,60)}`);
  }
});
...
test('OSA ต้องแยกภาวะแทรกซ้อนโดยตรงออกจากโรคที่แค่มีความเสี่ยงสูงขึ้น', () => {
  ...
  for(const s of ['CPAP', 'ออกซิเจนต่ำขณะหลับ', 'ความดันหลอดเลือดปอดสูง', 'หลับใน'])
...
test('การ์ดรายภาวะต้องมีครบทั้งสามสี และตัวเรนเดอร์ต้องรองรับ', () => {
  const i = html.indexOf('function faqBodyHtml(txt){');
  const sec = html.slice(i, html.indexOf('function faqHaystack', i));
  assert.ok(sec.includes('faq-case-'), 'ตัวเรนเดอร์ยังไม่รองรับการ์ดรายภาวะ');
  for(const cls of ['faq-case-bad','faq-case-mid','faq-case-ok'])
    assert.ok(html.includes('.' + cls + '{'), `ขาดสไตล์ ${cls}`);
  const cards = html.match(/^@[RYG] /gm) || [];
  assert.ok(cards.length >= 60, `การ์ดรายภาวะเหลือ ${cards.length} ใบ น้อยผิดปกติ`);
});
